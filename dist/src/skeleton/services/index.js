...
function registerService(getOptions) {
    return {
        register: (app) => {
            ...
            service['entityName'] = options.name;
            ...
            router.patch('/:id', middlewares_1.setMethod('patch'), ...app.get('appMiddlewares')['before']['update'].map(utils_1.asyncRoute), ...middlewareBefore['update'].map(utils_1.asyncRoute), utils_1.asyncRoute(wrapper.update()), ...middlewareAfter['update'].map(utils_1.asyncRoute), ...app.get('appMiddlewares')['after']['update'].map(utils_1.asyncRoute));
            ...
            app.useService(options.name, service);
        },
    };
}
